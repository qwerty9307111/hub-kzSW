from pathlib import Path
import sys

ROOT_DIR = Path(__file__).resolve().parent.parent
sys.path.insert(0, str(ROOT_DIR))

from src.retrieval.config import RetrievalConfig
from src.retrieval.factory import RetrieverFactory
from src.llm.ollama_client import OllamaClient


FAISS_DIR = ROOT_DIR / "data" / "faiss_index"

EMBEDDING_MODEL = Path(
    r"C:\Users\sean\Downloads\LLM_modules\bge-large-zh-v1.5"
)

RERANKER_MODEL = Path(
    r"C:\Users\sean\Downloads\LLM_modules\bge-reranker-large"
)


def print_results(results):
    for i, r in enumerate(results, 1):
        print("\n" + "=" * 100)
        print(f"[{i}]")
        print(f"score        : {r.get('score')}")
        print(f"rerank_score : {r.get('rerank_score')}")
        print(f"type         : {r.get('retrieval_type')}")
        print(f"sources      : {r.get('retrieval_sources')}")
        print(f"book         : {r.get('book')}")
        print(f"chapter      : {r.get('chapter')}")
        print(f"page         : {r.get('start_page')} - {r.get('end_page')}")
        print("-" * 100)
        print(r.get("content", "")[:700])


def main():
    query = "吴梅如何理解词的气韵？"

    llm = OllamaClient(
        model="qwen2.5:7b"
    )

    config = RetrievalConfig(
        mode="rrf",
        top_k=5,

        dense_k=20,
        bm25_k=20,

        enable_multi_query=True,
        enable_hyde=True,
        enable_self_query=True,

        enable_rerank=True,
        rerank_candidate_k=30,
        rerank_top_k=5,
    )

    retriever = RetrieverFactory.create(
        faiss_dir=FAISS_DIR,
        embedding_model_path=EMBEDDING_MODEL,
        config=config,
        llm_client=llm,
        reranker_model_path=RERANKER_MODEL,
    )

    results = retriever.retrieve(query)

    print_results(results)


if __name__ == "__main__":
    main()
